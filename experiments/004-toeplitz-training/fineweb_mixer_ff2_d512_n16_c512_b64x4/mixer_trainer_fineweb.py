...
model = LanguageMixer(n_vocab, decoder_dim, n_layers).float().to(device)
#model = AutoencodingMixer(n_vocab, encoder_dim, n_layers, tokenized_length).float()
#model = MemoryMixer(n_vocab, encoder_dim, decoder_dim, n_layers, tokenized_length, compression=compression, combination_dim='token', n_heads=0).float()
...
output_dir = f'/home/bbadger/Desktop/fineweb_mixer_ff2_d{decoder_dim}_n{n_layers}_c{tokenized_length}_b64x4'
...
#trainer.train("/home/bbadger/Desktop/fineweb_ememory_mixer1024c4_d1024_n16_c1024_b16/checkpoint-152000")
trainer.train()
