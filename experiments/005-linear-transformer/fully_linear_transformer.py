...
class SelfAttention(nn.Module):
    def __init__(self, dim, heads, causal = False, dim_head = None, blindspot_size = 1, dropout = 0., attn_dropout = 0.):
        super().__init__()
        ...
        self.global_attn_fn = partial(causal_linear_attn, bucket_size = blindspot_size)
...
class LinearTransformerLM(nn.Module):
    def __init__(
        ...
        self.transformer = LinearAttentionTransformer(dim, depth, max_seq_len, heads = heads, dim_head = dim_head, causal = causal, ff_chunks = ff_chunks, ff_glu = ff_glu, ff_dropout = ff_dropout, attn_layer_dropout = attn_layer_dropout, attn_dropout = attn_dropout)
